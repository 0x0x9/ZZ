...
function ResultsDisplay({ result, onApply, onReset }: { result: ConfigurePcOutput, onApply: (config: Configuration, modelName: string) => void, onReset: () => void }) {
    ...
    // Simplistic mapping, would ideally come from product data
    const recommendedProductId = result.modelName.toLowerCase().includes("fi") ? 1 : result.modelName.toLowerCase().includes("alpha") ? 10 : 9;
    ...
    return (
        ...
        >
            <div className="text-center">
                <h3 className="text-2xl font-bold">Configuration Recommandée : {result.modelName}</h3>
                <p className="text-muted-foreground mt-2 max-w-2xl mx-auto">{result.justification}</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {configArray.map(([key, value]) => {
                    ...
                    return (
                        <div key={key} className="flex items-center gap-4 text-left p-4 rounded-xl bg-background/50 border border-border/50">
